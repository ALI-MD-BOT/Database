...
B = '\033[1;34m' ; C = '\033[1;36m' ; W = '\033[1;37m' ; RESET = '\033[0m'
...
def jbk_banner():
    clear()
    # Bada ASCII delete karke simple header banaya hai
    print(f"{C}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━")
    ...
    try:
        ...
        if res:
            ...
            for key, val in res.items():
                k_low = str(key).lower()
                if any(x in k_low for x in ["howler", "developer", "status", "count", "query", "success"]): continue 
                
                if isinstance(val, list):
                    for item in val:
                        for k, v in item.items():
                            if any(x in str(k).lower() for x in ["howler", "developer"]): continue
                            print(f"{C}┃ {G}{k.upper():<12} {W}: {W}{str(v)[:25]:<25} {C}┃")
                            if "address" in k.lower() and v and "no" not in str(v).lower() and "null" not in str(v).lower(): 
                                target_address = v
                        print(f"{C}┢━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━┩")
                else:
                    print(f"{C}┃ {G}{key.upper():<12} {W}: {W}{str(val)[:25]:<25} {C}┃")
            ...
            if target_address and len(target_address) > 3:
                ...
                print(f"\n{R}[!] No valid map location found for this record.{RESET}")
        else:
...
def main():
    while True:
        ...
        print(f"    {W}[{G}02{W}] {C}JBK DARKWORKS CHANNEL")
